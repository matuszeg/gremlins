...
func TestGetTestArgs(t *testing.T) {
	...
	testCases := map[string]struct {
		...
		"should_not_include_tags_flag_when_build_tags_are_empty": {
			...
			want:              []string{"test", "-count=1", "-vet=off", "-timeout", "10s", "-failfast", "example.com/my/package"},
		},
		"should_include_tags_flag_when_build_tags_are_set": {
			buildTags:         "tag1,tag2",
			testExecutionTime: 10 * time.Second,
			pkg:               "example.com/my/package",
			want:              []string{"test", "-count=1", "-vet=off", "-tags", "tag1,tag2", "-timeout", "10s", "-failfast", "example.com/my/package"},
		},
		"should_pass_the_run_bound_verbatim_so_compile_time_is_not_charged_to_it": {
			testExecutionTime: 30 * time.Second,
			pkg:               "example.com/my/package",
			want:              []string{"test", "-count=1", "-vet=off", "-timeout", "30s", "-failfast", "example.com/my/package"},
		},
		...
		"should_include_cpu_flag_when_test_cpu_is_nonzero": {
			...
			testCPU:           4,
			pkg:               "example.com/my/package",
			want:              []string{"test", "-count=1", "-vet=off", "-timeout", "10s", "-failfast", "-cpu", "4", "example.com/my/package"},
		},
		...
		"should_use_dot_dot_dot_path_when_integration_mode_is_enabled": {
			...
			pkg:               "example.com/my/package",
			want:              []string{"test", "-count=1", "-vet=off", "-timeout", "10s", "-failfast", "./..."},
		},
		"should_include_all_flags_when_all_options_are_configured": {
			...
			integrationMode:   true,
			pkg:               "example.com/my/package",
			want:              []string{"test", "-count=1", "-vet=off", "-tags", "integration", "-timeout", "10s", "-failfast", "-cpu", "2", "./..."},
		},
	}
